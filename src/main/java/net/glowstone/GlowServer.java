...
public final class GlowServer implements Server {
	...
	/**
	 * Default entity worker count
	 */
	public static final int DEFAULT_ENTITY_WORKERS = 1;
	 ...
	 * Load loadBalancer.
	 */
	private ExecutorService loadBalancer;
	...
	public void start() {
		...
		// Start load loadBalancer
		loadBalancer = new ThreadPoolExecutor(1, config.getInt("server.entity-workers"), 30L, TimeUnit.SECONDS, new LinkedBlockingQueue<Runnable>());
		...
		commandMap.register(new SpawnCommand(this));
	...
	public GlowWorld createWorld(WorldCreator creator) {
		...
		world = new GlowWorld(this, creator.name(), creator.environment(), creator.seed(), new McRegionWorldStorageProvider(new File(creator.name())), creator.generator(), loadBalancer);
		worlds.add(world);
		return world;
	}

...
public final class GlowPlayer extends GlowHumanEntity implements Player, InventoryViewer {
	...
	public void setSneaking(boolean sneak) {
		...
		final byte shift = 0x02;
		Byte metadataUpdate = 0;
		try {
			metadataUpdate = (Byte) getMetadata(0).getValue();
		} catch (IndexOutOfBoundsException ex) {
			// This is pretty normal: if no metadata had been set before, there WILL be an IndexOutOfBounds error.
		}

		if (sneak) {
			metadataUpdate = (byte) (metadataUpdate | shift);
		} else {
			metadataUpdate = (byte) (metadataUpdate & ~shift);
		}

		setMetadata(new Parameter<Byte>(Parameter.TYPE_BYTE, 0, metadataUpdate));
	...
	public void setSprinting(boolean sprint) {
		if (EventFactory.onPlayerToggleSprint(this, sprint).isCancelled()) {
			return;
		}
		this.sprinting = sprint;

		final byte shift = 0x08;
		Byte metadataUpdate = 0;
		try {
			metadataUpdate = (Byte) getMetadata(0).getValue();
		} catch (IndexOutOfBoundsException ex) {
